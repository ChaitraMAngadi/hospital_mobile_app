...
//       Uri.parse('${Constants.baseUrl}/api/v1/doctor/warmup'),
//       headers: {
...
//                   // ── Cancel button (analyzing state only) ──
//                   // if (isAnalyzing) ...[
//                   //   const SizedBox(height: 16),
//                   //   GestureDetector(
//                   //     onTap: _showCancelAnalyzingConfirm,
//                   //     child: Container(
//                   //       padding: const EdgeInsets.symmetric(
...
//       doctorprovider.refreshtoken(context);
//         try {
...
Future<void> _pickFromCamera() async {
  ...
  // ✅ Re-check after async picker call, in case state changed
  if (selectedFiles.length >= 5) {
    if (!mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text("Maximum 5 files allowed")),
...
Future<void> pickFiles() async {
  ...
  if (result != null) {
    ...
    final oversizedFiles =
        allowedFiles.where((file) => file.size > maxFileSizeBytes).toList();

    if (oversizedFiles.isNotEmpty) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text(
          "Files >5MB skipped: ${oversizedFiles.map((f) => f.name).join(', ')}",
        ),
      ));
    ...
    final newFiles = allowedFiles.map((file) => File(file.path!)).toList();
    setState(() {
      selectedFiles.addAll(newFiles);
    });
