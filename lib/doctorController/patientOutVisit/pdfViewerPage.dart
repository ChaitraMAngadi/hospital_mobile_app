...
//       final String url =
//           "${Constants.baseUrl}/api/v1/hospitaldoctor/getdownloaddetailspdf/${widget.patientId}/${widget.complaintId}/$selectedLanguage";
...
//         name: "${widget.patientId}_Report_${languages[selectedLanguage]}.pdf",
//       );
//     } catch (error) {
...
class _PdfViewerPageState extends State<PdfViewerPage> {
  ...
  // Key format: "patientId_complaintId_languageCode"
  static final Map<String, Uint8List> _pdfCache = {};

  /// Unique key for the currently selected language.
  String get _cacheKey =>
      '${widget.patientId}_${widget.complaintId}_$selectedLanguage';
  ...
  // ─── Token Refresh ────────────────────────────────────────────────────────
  // NOTE: Field/endpoint names below are assumed to mirror the doctor app's
  // pattern (Constants.doctorrefreshtoken + hospitaldoctor refresh route).
  // Please verify these against your actual Constants class / backend route.
  ...
  void _onLanguageChanged(String? newLanguage) {
    ...
    final pendingKey =
        '${widget.patientId}_${widget.complaintId}_$newLanguage';
  ...
  Future<void> _loadPdfData() async {
    ...
    try {
      ...
      final String url =
          '${Constants.baseUrl}/api/v1/hospitaldoctor/getdownloaddetailspdf'
          '/${widget.patientId}/${widget.complaintId}/$selectedLanguage';
        ...
        actions: [
          ...
          Container(
            ...
            child: DropdownButtonHideUnderline(
              child: DropdownButton<String>(
                value: selectedLanguage,
                // ✅ null disables the dropdown while loading — this is the
                // exact behaviour ported over from the first file.
              ...
              children: [
                ...
                Container(
                  ...
                  child: ElevatedButton.icon(
                    ...
                    // ✅ also disabled while loading, same as first file
                    onPressed:
  ...
  Widget _buildPdfViewer() {
    ...
    if (pdfBytes != null) {
      return PdfPreview(
        ...
        maxPageWidth: MediaQuery.of(context).size.width,
        pdfFileName:
            '${widget.patientId}_Report_${languages[selectedLanguage]}.pdf',
      );
    }
